...
namespace fm
{
	...
	*/
	template<typename T>
	void ReadDirectoryWS(const std::string, std::vector<T>&, const std::vector<std::string> & = {}, bool = false, bool = false);
	...
	*/
	...
	/**
		Zmienia atrybut ukrycia pliku
		@param sciezka do pliku w ktorym zmieniamy atybut
		@param bool czy zmienic na ukryty czy zmienic na jawny
		@return nic nie zwraca
	*/
	void HiddenFile(std::string, bool);
	/**
		HiddenFile z opcja wypisania logow
	*/
	void HiddenFileLog(std::string, bool, std::ostream&);
	/**
		HiddenFile dla nazw plikow okreslonych jako wstringi
	*/
	void HiddenFileW(std::wstring, bool);
	/**
		HiddenFileW z opcja wypisania logow
	*/
	void HiddenFileWLog(std::wstring, bool , std::wostream&);
