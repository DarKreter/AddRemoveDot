...
namespace fm
{
    ...
    void HiddenFile(std::string name, bool makeHidden)
    {
        string command = ((makeHidden == true) ? "attrib +h \"" : "attrib -h \"") + name + "\"";
        unsigned short error = system(command.c_str());

        if (error)
            throw Error_t{ "Error in making file hidden.\nFile: " + name + "\nError log: " + to_string(error) };
    }

    void HiddenFileW(std::wstring name, bool makeHidden)
    {
        wstring command = ((makeHidden == true) ? L"attrib +h \"" : L"attrib -h \"") + name + L"\"";
        unsigned short error = _wsystem(command.c_str());

        if (error)
        {
            string err = sm::WSTS(L"Error in making file hidden.\nFile: " + name + L"\nError log: ");
            err += to_string(error);
            throw Error_t{ err };
        }
    }

    void HiddenFileWLog(std::wstring name, bool makeHidden, std::wostream& strumien)
    {
        strumien << "-------------MAKING FILE HIDDEN-------------"
            << endl << "File:\t" << name << endl << "MakeHidden: " << makeHidden << endl;
        Sleep(LOG_SLEEP);
    ...
    void HiddenFileLog(std::string name, bool makeHidden, std::ostream& strumien)
    {
        strumien << "-------------MAKING FILE HIDDEN-------------"
            << endl << "File:\t" << name << endl << "MakeHidden: " << makeHidden << endl;
        Sleep(LOG_SLEEP);
